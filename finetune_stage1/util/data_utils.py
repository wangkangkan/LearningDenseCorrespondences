...
def parse_example_proto_our_3dpoint(example_serialized, pointnum, pointnum2=3000):
    ...
    """
    feature_map = {
        'stage2/x': tf.FixedLenFeature((pointnum, 1), dtype=tf.float32),
        'stage2/y': tf.FixedLenFeature((pointnum, 1), dtype=tf.float32),
        'stage2/z': tf.FixedLenFeature((pointnum, 1), dtype=tf.float32),
        # 'feat1': tf.FixedLenFeature((pointnum, 101), dtype=tf.float32),
    ...
    # vis = tf.cast(features['image/visibility'], dtype=tf.float32)
    x = tf.cast(features['label/x'], dtype=tf.float32)
    y = tf.cast(features['label/y'], dtype=tf.float32)
    z = tf.cast(features['label/z'], dtype=tf.float32)

    label = tf.concat([x, y, z], 1)

    pointsetx = tf.cast(features['stage2/x'], dtype=tf.float32)
    pointsety = tf.cast(features['stage2/y'], dtype=tf.float32)
    pointsetz = tf.cast(features['stage2/z'], dtype=tf.float32)
    pointset = tf.concat([pointsetx, pointsety, pointsetz], 1)


    # input_pc_x = tf.cast(features['frame2/x'], dtype=tf.float32)
    # input_pc_y = tf.cast(features['frame2/y'], dtype=tf.float32)
    # input_pc_z = tf.cast(features['frame2/z'], dtype=tf.float32)
    ...
    return pointset, label#, input_pc#, pose, shape, translation
...
def get_all_files(dataset_dir, datasets, split='train'):
    ...
    data_dir = "/test/17048/temp_result/cape_train/tfrecord_stage2/tfrecord_stage2/" + "*.tfrecord"
