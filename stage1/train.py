...
parser.add_argument('--data', default='../flownet3d/human_stage1_cape/', help='Dataset directory [default: data_preprocessing/data_processed_maxcut_35_20k_2k_8192]')
parser.add_argument('--log_dir', default='log_train_cape_new', help='Log dir [default: log_train]')
parser.add_argument('--learning_rate', type=float, default=0.0001, help='Initial learning rate [default: 0.001]')
parser.add_argument('--num_point', type=int, default=6890, help='Point Number [default: 2048]')
parser.add_argument('--max_epoch', type=int, default=100, help='Epoch to run [default: 151]')
...
def parse_example(example_serialized, pointnum=6890, pointnum2=FRM2_NUM_POINT, k=300):
...
def train_one_epoch(sess, ops, saver):
    ...
    for batch_idx in range(0, num_batches):
        ...
        if step % allstep == 0:
            print('loss_avg:', loss_sum/allstep)
            loss_sum = 0
        if step % 2000 == 0:
            save_path = saver.save(sess, os.path.join(LOG_DIR, "model.ckpt"))
            print('saved!!!')
    return loss_batch / num_batches, step // num_batches - 1
