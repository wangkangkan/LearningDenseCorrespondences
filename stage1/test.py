...
from write2obj import read_obj, read_obj_sample, write_to_obj
...
MODEL_PATH = './log_train_cape_new/model.ckpt'
    ...
    if train_flag:
        with tf.Graph().as_default():
            with tf.device('/gpu:'+str(GPU_INDEX)):
                ...
                idx_1723 = down[:, 1]
                idx_1723_tf = tf.constant(idx_1723, dtype=tf.int32)
...
def train_one_epoch(sess, ops, saver):
    ...
    batch_data[:, 0:NUM_POINT, 0:3] = np.tile(frame1, reps=[BATCH_SIZE, 1, 1])
    # batch_data[:, 0:NUM_POINT, 3:] = np.tile(feat1, reps=[BATCH_SIZE, 1, 1])

    global_count = 0
    global_count2 = 0
    keyword = 'taichi'

    SAVE_DIR = "/test/hks_part3_faust/cape/cape_test/stage5/"
    if not os.path.exists(SAVE_DIR):
        os.makedirs(SAVE_DIR)
    allfilename_feat2 = natsort.natsorted(glob.glob("/test/hks_part3_faust/cape/cape_test/input_pc/*.txt"))
    allfilename_feat2 = [item for item in allfilename_feat2 if 'normal' not in item]
    ...
    for batch_idx in range(0, num_batches):
        ...
        for i, line in enumerate(xyz2list):
            ...
            batch_data[i, NUM_POINT:, 0:3] = np.loadtxt(xyz2dir)

            batch_label[i, :, :3] = read_obj('../fittingcode_ourman2/cape/example1_groundtruth_fit.obj')[0]
            global_count2 += 1
