...
def convert_to_example_our_correspondence(depth_imagedata, flow_imagedata1, match_imagedata1, flow_imagedata2, match_imagedata2,flow_imagedata3, match_imagedata3,flow_imagedata4, match_imagedata4, height, width):
    ...
    feat_dict = {
        ...
        'image/depth': bytes_feature(tf.compat.as_bytes(depth_imagedata)),
        'image/flow1': bytes_feature(tf.compat.as_bytes(flow_imagedata1)),
        'image/match1': bytes_feature(tf.compat.as_bytes(match_imagedata1)),
        'image/flow2': bytes_feature(tf.compat.as_bytes(flow_imagedata2)),
        'image/match2': bytes_feature(tf.compat.as_bytes(match_imagedata2)),
        'image/flow3': bytes_feature(tf.compat.as_bytes(flow_imagedata3)),
        'image/match3': bytes_feature(tf.compat.as_bytes(match_imagedata3)),
        'image/flow4': bytes_feature(tf.compat.as_bytes(flow_imagedata4)),
        'image/match4': bytes_feature(tf.compat.as_bytes(match_imagedata4)),
    }
...
def convert_to_example_our3(image_data, image_path, height, width, labelimage, label3d, label):
    """Build an Example proto for an image example.
    Args:
      image_data: string, JPEG encoding of RGB image;
      image_path: string, path to this image file
    ...
    feat_dict = {
        ...
        'image/visibility': float_feature(label[:,2].astype(np.float)),
        'labelimage/encoded': bytes_feature(tf.compat.as_bytes(labelimage)),
    ...
    feat_dict.update({
        'mosh/pose':
            float_feature(label3d[:72].astype(np.float)),
        'mosh/shape':
            float_feature(label3d[72:82].astype(np.float)),
        'camera/rotation':
            float_feature(label3d[82:85].astype(np.float)),
        'camera/translation':
            float_feature(label3d[85:].astype(np.float)),
    })
...
def convert_to_example_our2(image_data, image_path, height, width, labelimage, label3d, label):
    """Build an Example proto for an image example.
    Args:
      image_data: string, JPEG encoding of RGB image;
      image_path: string, path to this image file
    ...
    feat_dict = {
        ...
        'image/x': float_feature(label[:,0].astype(np.float)),
        'image/y': float_feature(label[:,1].astype(np.float)),
        'image/z': float_feature(label[:,2].astype(np.float)),
        'image/visibility': float_feature(label[:,3].astype(np.float)),
        'labelimage/encoded': bytes_feature(tf.compat.as_bytes(labelimage)),
        'image/format': bytes_feature(tf.compat.as_bytes(image_format)),
        'image/filename': bytes_feature(
            tf.compat.as_bytes(basename(image_path))),
...
def convert_to_example_our_3dpoint(pointset,  label, initpara):
    """Build an Example proto for an image example.
    Args:
      image_data: string, JPEG encoding of RGB image;
      image_path: string, path to this image file
    ...
    feat_dict = {
        ...
        'image/z': float_feature(label[:,2].astype(np.float)),
        'image/visibility': float_feature(label[:,3].astype(np.float)),
        'image/pointsetx': float_feature(pointset[:,0].astype(np.float)),
        'image/pointsety': float_feature(pointset[:, 1].astype(np.float)),
        'image/pointsetz': float_feature(pointset[:, 2].astype(np.float)),
        'initpara/initparameter':float_feature(initpara.astype(np.float)),
    }
    # feat_dict.update({
    #     'mosh/pose':
    #         float_feature(label3d[:72].astype(np.float)),
    ...
    #         float_feature(label3d[72:82].astype(np.float)),
    #     'camera/translation':
    #         float_feature(label3d[82:].astype(np.float)),
    # })
...
def convert_to_example_our_depth(image_data, label3d, label):
    """Build an Example proto for an image example.
    Args:
      image_data: string, JPEG encoding of RGB image;
      image_path: string, path to this image file
...
def convert_to_example_our_3dpoint_initpara(pointset, label3d, label, initpara):
    """Build an Example proto for an image example.
    Args:
      image_data: string, JPEG encoding of RGB image;
      image_path: string, path to this image file
...
def convert_to_example_our_3dpoint_RT(pointset, label3d, label):
    """Build an Example proto for an image example.
    Args:
      image_data: string, JPEG encoding of RGB image;
      image_path: string, path to this image file
...
def convert_to_example_our2_withoutR(image_data, image_path, height, width, labelimage, label3d, label):
    """Build an Example proto for an image example.
    Args:
      image_data: string, JPEG encoding of RGB image;
      image_path: string, path to this image file
...
def convert_to_example_our(image_data, image_path, height, width, labelimage, label3d):
    """Build an Example proto for an image example.
    Args:
      image_data: string, JPEG encoding of RGB image;
      image_path: string, path to this image file
    ...
    feat_dict = {
        'image/height': int64_feature(height),
        'image/width': int64_feature(width),
        # 'image/x': float_feature(label[0, :, 0].astype(np.float)),
        # 'image/y': float_feature(label[0, :, 1].astype(np.float)),
        # 'image/z': float_feature(label[0, :, 2].astype(np.float)),
        # 'image/visibility': int64_feature(label[0, :, 3].astype(np.int)),
        'labelimage/encoded': bytes_feature(tf.compat.as_bytes(labelimage)),
        'image/format': bytes_feature(tf.compat.as_bytes(image_format)),
        'image/filename': bytes_feature(
            tf.compat.as_bytes(basename(image_path))),
