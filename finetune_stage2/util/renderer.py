...
def draw_skeleton(input_image, joints, draw_edges=True, vis=None, radius=None):
    ...
    for child in range(len(parents)):
        ...
        if vis is not None and vis[child] == 0:
            continue
        if draw_edges:#
            cv2.circle(image, (point[0], point[1]), radius, (int(colors['white'][0]),int(colors['white'][1]),int(colors['white'][2])),
                       -1)
            cv2.circle(image, (point[0], point[1]), radius - 1,
                       (int(colors[jcolors[child]][0]), int(colors[jcolors[child]][1]), int(colors[jcolors[child]][2])), -1)
        else:
        ...
        if draw_edges and pa_id >= 0:
            ...
            cv2.circle(image, (point_pa[0], point_pa[1]), radius - 1,
                       (int(colors[jcolors[pa_id]][0]), int(colors[jcolors[pa_id]][1]), int(colors[jcolors[pa_id]][2])), -1)
            if child not in ecolors.keys():
                print('bad')
                import ipdb
                ipdb.set_trace()
            cv2.line(image, (point[0], point[1]), (point_pa[0], point_pa[1]),
                     (int(colors[ecolors[child]][0]), int(colors[ecolors[child]][1]), int(colors[ecolors[child]][2])), radius - 2)
