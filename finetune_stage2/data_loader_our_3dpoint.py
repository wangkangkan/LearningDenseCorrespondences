...
_3D_DATASETS = ['tran_cape_20w']


def num_examples(datasets):
    _NUM_TRAIN = {
        'tran_tfrecords_30w': 300000,
        'tran_cape_20w': 200000,
        'lsp': 1000,
        'lsp_ext': 10000,
        'mpii': 20000,
        'h36m': 312188,
...
class DataLoader(object):
    ...
    def get_loader_w3d(self):
        ...
        # pointset, label = self.read_data(
        #     fqueue_yes3d)
        smpl, label, input_pc, normal, paras, stage2, joint_and_mask = self.read_data(
            fqueue_yes3d)

        min_after_dequeue = 2000
        capacity = min_after_dequeue + 3 * self.batch_size

        # pack_these = [pointset, label]
        # pack_name = ['pointset', 'label']
        pack_these = [smpl, label, input_pc, normal, paras, stage2, joint_and_mask]
        pack_name = ['smpl', 'label', 'input_pc', 'normal', 'paras', 'stage2', 'joint_and_mask']
    ...
    def read_data(self, filename_queue):
        with tf.name_scope(None, 'read_data', [filename_queue]):
            ...
            # pointset, label = data_utils.parse_example_proto_our_3dpoint(
            #     example_serialized, 1723)
            smpl, label, input_pc, normal, paras, stage2, joint_and_mask = data_utils.parse_example_proto_our_3dpoint(
                example_serialized, 1723)
            ...
            # modelrotations = batch_rodrigues(tf.reshape(pose, [-1, 3]))
            # translation = tf.reshape(translation, [-1])
            # Label 3d is:
            #   [rotations, shape-beta, 3Djoints]
            #   [216=24*3*3, 10, 42=14*3]
            # label3d = tf.concat(
            #     [tf.reshape(modelrotations, [-1]), shape,translation], 0)

            # label should be K x 4
            #label = tf.transpose(label)

            return smpl, label, input_pc, normal, paras, stage2, joint_and_mask
